# Content Lab — Design Seed (Identity)

This folder is **not** the content lab. It is the lab's design record and its **build seed**:
everything needed to build the blog-first social-media content lab from scratch and plug it into
whatever workspace it lands in. The design was locked 2026-07-05; the lab has **not been built
yet** — it waits on the human's go.
...
| File | Purpose |
|------|---------|
| `SPEC.md` | The build contract: requirements, folder structure, template scaffolds, and the Build & Integration Protocol |
...
- When building, follow `SPEC.md`'s **Build & Integration Protocol** section exactly — it covers
  where to build, what existing material to preserve (including the legacy `Writing/` and
  `LinkedIn/` folders this design replaces), and how to update the parent workspace's
