...
- Work only from the actual findings in `stages/03_parsing/output/compliance_report.md` (canonical path from the EVD folder root). Do not invent findings or assume violations beyond what is documented there.
...
- **Output location**: Write the remediation plan to `stages/04_remediation/output/remediation_plan.md` (canonical path from the EVD folder root).
