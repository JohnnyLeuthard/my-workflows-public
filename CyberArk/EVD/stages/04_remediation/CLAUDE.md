...
- Work only from the actual findings in `../03_parsing/output/compliance_report.md`. Do not invent findings or assume violations beyond what is documented there.
- Every action item must reference the specific `naming_standards.md` rule that was violated.
...
- Never generate psPAS cmdlets or PowerShell code in this stage. That belongs in `../../../psPAS/stages/01_planning/`. If the user asks for scripts, redirect them there.
- Do not execute any changes. This stage produces a decision document only.
- Stop after writing `output/remediation_plan.md`. Wait for human review.
