...
- **Self-contained sub-workspaces**: Each sub-workspace (EVD, psPAS, cyberark-api, CCP, api-evaluation) carries its own references in its `references/` folder. Point to the owning sub-workspace's `references/` instead of duplicating. The `_config/` folder holds only cross-sub-workspace pointers (e.g., `environment.md`).
...
- **CCP is a staged design/test workspace**: CCP work starts with config modeling and request cataloging before any URI builder or endpoint test runs. Live credential retrieval tests must be explicitly approved and must not store returned secrets in repo artifacts.
- **api-evaluation2 is a reference library, not an executor**: `api-evaluation2/` documents the REST API endpoints per PVWA version. When working with API specs, load `api-evaluation2/CLAUDE.md` for routing. Live API calls belong in `cyberark-api` or `psPAS`. Only load api-evaluation2 context when actually referencing API documentation.
